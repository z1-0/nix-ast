...
> A Nix library for parsing, inspecting, transforming, and evaluating Nix Abstract Syntax Trees (AST), powered by Haskell's [hnix](https://github.com/haskell-nix/hnix) parser.

`nix-ast` bridges two worlds: it exposes a full Nix AST API directly in Nix itself (via IFD — Import From Derivation), while using hnix's battle-tested Haskell parser and evaluator under the hood. This means you can parse any Nix expression into a structured AST, traverse and transform it with pure Nix functions, render it back to source code, and even evaluate it — all from within your Nix expressions.
