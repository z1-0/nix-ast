let
  ...
  children = node:
    let tag = getExprKind node; in
    if tag == "Abs" then [ node.body ]
    else if tag == "App" then [ node.func node.arg ]
    else if tag == "Assert" then [ node.cond node.body ]
    else if tag == "Binary" then [ node.left node.right ]
      ...
      in builtins.map (p: p.expr) interpolations
    else if tag == "SynHole" then []
    else if tag == "Unary" then [ node.arg ]
    else if tag == "With" then [ node.namespace node.body ]
    else [];

  rebuild = node: cs:
    let tag = getExprKind node; in
    if tag == "Abs" then node // { body = builtins.head cs; }
    ...
    let
      go = n:
        if pred n then n
          ...
          let childNodes = children n; in
          if builtins.length childNodes == 0 then null
          else
            let
              search = i:
                if i >= builtins.length childNodes then null
                else
                  let r = go (builtins.elemAt childNodes i); in
                  if r != null then r else search (i + 1);
            in
            search 0;
    in
    go node;
  ...
  collect = f: node:
    let
      go = n: acc:
        let
          val = f n;
          matches = if val != null then [val] else [];
          childNodes = children n;
          childResults = builtins.map (c: go c []) childNodes;
        in
        acc ++ matches ++ (builtins.concatLists childResults);
    ...
    let
      go = a: n:
        let
          ...
          childNodes = children n;
        in
        if builtins.length childNodes == 0 then a1
