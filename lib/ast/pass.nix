...
in
{
  rename = old: new: c.rewrite (node:
    if s.isSym node && s.getSymName node == old
    then s.mkSym new
    else node
  );

  replaceString = old: new: c.rewrite (node:
    if s.isStr node
    then
      let
  ...
  inline = name: ast:
    let
      ...
      findInBindings = node:
        let bindings =
          if s.isLet node then s.getLetBindings node
          else if s.isSet node then s.getSetBindings node
          else [];
            found = builtins.filter (b: b ? tag && b.tag == "NamedVar" && s.getNamedVarAttrPath b == name) bindings;
        in if found != [] then builtins.head found else null;
      findBinding = node:
        let result = findInBindings node;
        in if result != null then result
        else
          let childNodes = c.children node;
              search = i:
                if i >= builtins.length childNodes then null
                else
                  let r = findBinding (builtins.elemAt childNodes i);
                  in if r != null then r else search (i + 1);
          in search 0;
      binding = findBinding ast;
      value = if binding != null then s.getNamedVarValue binding else null;
      step1 = if uses != 1 || value == null then ast
              else c.rewrite (n:
                if s.isSym n && s.getSymName n == name
                then value
                else n
              ) ast;
      step2 = if uses != 1 || value == null then step1
              else c.rewrite (n:
                if s.isLet n
                then
                  let remaining = builtins.filter
                    (b: !(b ? tag && b.tag == "NamedVar" && s.getNamedVarAttrPath b == name))
                    (s.getLetBindings n);
                  in if remaining == [] then s.getLetBody n else n // { bindings = remaining; }
                else n
              ) step1;
    in step2;

  hoistLet = c.rewrite (node:
    if s.isLet node && s.isLet (s.getLetBody node)
    then
      let inner = s.getLetBody node;
      in s.mkLet (s.getLetBindings node ++ s.getLetBindings inner) (s.getLetBody inner)
    else node
  );

  flattenSets = c.rewrite (node:
