let
  ...
  assertType = funcName: argName: type: val:
    if type.check val
    then val
    else throw "TypeError in ${funcName}: Argument '${argName}' expected type '${type.name}', but got: ${builtins.toJSON val}";
  ...
  mkDoubleQuotedNode = parts: mkNode "DoubleQuoted" {contents = assertType "mkDoubleQuoted" "parts" (t.listOf t.AntiquotedText) parts;};
  mkIndentedNode = indent: parts:
    mkNode "Indented" {
...
in rec {
  ...
  getExprKind = node:
    if builtins.isAttrs node && node ? tag
    then node.tag
  ...
  mkList = items:
    mkNode "List" {
      contents = assertType "mkList" "items" (t.listOf t.Expr) items;
    };

  mkLiteralPath = path:
    mkNode "LiteralPath" {
      contents = assertType "mkLiteralPath" "path" t.textVal path;
    };

  mkSelect = defaultValue: expr: selectPath:
    mkNode "Select" {
  ...
  mkSym = name:
    mkNode "Sym" {
      contents = assertType "mkSym" "name" t.textVal name;
    };

  mkSynHole = name:
    mkNode "SynHole" {
      contents = assertType "mkSynHole" "name" t.textVal name;
    };
  ...
  mkInherit = scope: names:
    mkNode "Inherit" {
      scope = assertType "mkInherit" "scope" (t.maybe t.Expr) scope;
      names = assertType "mkInherit" "names" (t.listOf t.textVal) names;
    };

  mkNamedVar = attrPath: value:
    mkNode "NamedVar" {
      attrPath = assertType "mkNamedVar" "attrPath" (t.listOf t.KeyName) attrPath;
      value = assertType "mkNamedVar" "value" t.Expr value;
    };

  mkDynamicKey = content:
    mkNode "DynamicKey" {
      contents = assertType "mkDynamicKey" "content" t.String content;
    };

  mkStaticKey = keyName:
    mkNode "StaticKey" {
      contents = assertType "mkStaticKey" "keyName" t.textVal keyName;
    };

  mkParam = name:
    mkNode "Param" {
      contents = assertType "mkParam" "name" t.textVal name;
    };

  mkParamSet = paramSetName: params: variadic:
    mkNode "ParamSet" {
  ...
  mkAntiquoted = expr: mkNode "Antiquoted" {contents = assertType "mkAntiquoted" "expr" t.Expr expr;};
